...
import Animated, { useSharedValue, useAnimatedStyle, Easing, 
  withTiming, withSequence, withDelay, interpolateColor } from 'react-native-reanimated'
...
const AnimatedTaskLabel = memo((props: Props) => {
  ...
  const strikethroughAniamtedStyles = useAnimatedStyle(
    () => ({
  ...
  useEffect(() => {
    ...
    if (strikethrough) {
      ...
      textColorProgress.value = withDelay(
        1000, withTiming(1, {duration: 400, easing})
      )
  ...
  return (
    <Pressable onPress={onPress}>
      <AnimatedHStack alignItems="center" style={[hstackAnimatedStyles]}>
        ...
        <AnimatedBox position="absolute" h={1} borderBottomWidth={1} style={[strikethroughAniamtedStyles]}/>
      </AnimatedHStack>
    </Pressable>
  )
})
